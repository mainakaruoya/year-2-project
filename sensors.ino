...
int pirState = LOW;             // for both sensors, we start by assuming that their state is 'no motion detected'
int pirTwoState = LOW;         
...
void setup() {
  ...
  pinMode(inputPin, INPUT);     // declare sensor pins as input (this one and the one below it)
...
void loop(){
  ...
  if ((val == HIGH) && (val2 == HIGH)) {            // check if the input is HIGH
    ...
    if ((pirState == LOW) && (pirTwoState == LOW)) {
      ...
      Serial.println(data);
      // We only want to print on the output change, not state change
      pirState = HIGH;
      pirTwoState = HIGH;
    }
  } else {
